...
public class RotatingLight : MonoBehaviour {
	...
	void Update ()
	{
		...
		if(isRotateX)
		{
			transform.Rotate(myRotationSpeed * Time.deltaTime * posOrNeg, 0, 0);//rotates on X axis
			//Debug.Log("You are rotating on the X axis");	
		}
		//  Toggles Y Rotation
		if(isRotateY)
		{
			transform.Rotate(0, myRotationSpeed * Time.deltaTime * posOrNeg, 0);//rotates on Y axis
			//Debug.Log("You are rotating on the Y axis");
		}
		//  Toggles Z Rotation
		if(isRotateZ)
		{
			transform.Rotate(0, 0, myRotationSpeed * Time.deltaTime * posOrNeg);//rotates on Z axis
